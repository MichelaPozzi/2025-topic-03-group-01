...
***********************************************************************************************************
```{r}
#Defines a function gauss_fit that takes three arguments:
  #pos: Row index to select data from table
  #table_mean: Table with Gaussian fit parameters
  #table_rep: Table with raw measurement values
gauss_fit <- function(pos, table_mean, table_rep) {
  #Extracts 25 y-values from table_rep
  df.y <- c(as.numeric(table_rep[pos,1:25]))
  #Creates x-values from 1 to 25
  y <- c(1:25)
  #Combines x and y-values into a data frame for model fitting
  data <- data.frame(y = y, df.y = df.y)
  #Extracts Gaussian parameters (c, mean, sigma) from table_mean and converts       them to numeric vectors
  ...
  #Determines how many peaks are present (how many sets of Gaussian parameters)
  n <- length(list_c)
  #Initializes empty numeric vector (for optimization starting values)
  vector <- numeric(0)
  #Creates data frame for storing fitted values
  dffit <- data.frame(y=seq(1, 25, 1))
  #Numeric vector from 1 to 25 computing the gaussian function
  z <- seq(1, 25)
  #Special case: If there is only 1 peak and amplitude c == 100, then the fit is forced to be 0
  if ((n==1) && (list_c == 100)) {dffit$df.y <- rep(0,25)} else {
    #Otherwise, if there is exactely one peak, continue with fitting
    if (n==1) {
      #Set initial values for optimization (amplitude, mean, sd)
      vector <- c(list_c[1],list_mean[1],list_sigma[1])
      #Performs Gaussian fitting using optim() with the BFGS method. Catches errors and returns empty vectors if fitting fails. The fitted parameters are rounded to 1 decimal place
        ##BFGS = Broyden-Fletcher-Goldfarb-Shanno -> Iterative method for solving optimization problem, to minimize a function. Usually done using method of steepest descent to find local minimizer.
...
```{r}
table.ctrl1.fit$fit_res <- sapply(vect, function(x) { gauss_fit_res(x, ctrl_3, norm_tables.ctrl$ctrl1) })
table.ctrl2.fit$fit_res <- sapply(vect, function(x) { gauss_fit_res(x, ctrl_3, norm_tables.ctrl$ctrl2) })
table.ctrl3.fit$fit_res <- sapply(vect, function(x) { gauss_fit_res(x, ctrl_3, norm_tables.ctrl$ctrl3) })
