...
***********************************************************************************************************
# Identifying shoulders
# "shoulders" are regions of elevated signal intensity that are not sharp enough to be detected as a "peak" by the peak detection function find_peaks — but still indicate meaningful protein presence.
...
#norm_tables.ctrl$rnase1 
#norm_tables.ctrl$rnase2 
#norm_tables.ctrl$rnase3
...
```{r}
# Beispiel-DataFrames
df1 <- data.frame(result = c("success", "failure", "success", "success", "failure"))
df2 <- data.frame(result = c("failure", "failure", "success", "failure", "failure", "success"))

# Zähle die Anzahl der "success" und die Gesamtanzahl
success1 <- sum(df1$result == "success")
n1 <- nrow(df1)

success2 <- sum(df2$result == "success")
n2 <- nrow(df2)

# Proportionstest (zweiseitig)
prop.test(x = c(success1, success2), n = c(n1, n2), correct = FALSE)
...
```{r}
row_data1 <- as.numeric(table.ctrl2.fit["PRKDC_HUMAN", ])
row_data2 <- as.numeric(table.rnase2.fit["PRKDC_HUMAN", ])
...
`
```{r}
df_numeric_ctrl1 <- as.data.frame(lapply(table.ctrl1.fit.fine, function(x) as.numeric(as.character(x))))

anyNA(df_numeric_ctrl1)

df_numeric_rnase1 <- as.data.frame(lapply(table.rnase1.fit.fine, function(x) as.numeric(as.character(x))))

colnames(df_numeric_ctrl1) <- colnames(table.ctrl1.fit.fine)
colnames(df_numeric_rnase1) <- colnames(table.rnase1.fit.fine)
```


```{r}
# T-Test pro Zeilenpaar
t_test_ergebnisse2 <- lapply(seq_len(nrow(df_numeric_ctrl1)), function(i) {
  x <- as.numeric(df_numeric_ctrl1[i, ])
  y <- as.numeric(df_numeric_rnase1[i, ])

  # t-Test
    t_res <- t.test(x, y)
    data.frame(
      zeile = rownames(df_numeric_ctrl1)[i],
      p_wert = t_res$p.value,
      mittelwert_df_ctrl1 = mean(x, na.rm = TRUE),
      mittelwert_df_rnase1 = mean(y, na.rm = TRUE),
      t_wert = t_res$statistic,
      df = t_res$parameter
    )
  })
...
row_data5 <- as.numeric(ctrl_mean["PRKDC_HUMAN", 1:25])
row_data6 <- as.numeric(rnase_mean["PRKDC_HUMAN", 1:25])
...
#Subtable of rnase is larger than the Ctrl Subtable
...
rows_to_keep <- setdiff(rows_rnase1, rows_rnase2)

# table1 ohne die gemeinsamen Reihen
rnase_mean_filtert <- rnase_mean[rows_to_keep,]
...
# table1 ohne die gemeinsamen Reihen
ctrl_mean_filtert <- ctrl_mean[rows_to_keep,]
```

```{r}
df_ctrl <- ctrl_mean_filtert[1:25] # Gruppe A
df_rnase <- rnase_mean_filtert[1:25] # Gruppe B
...
colnames(df_numeric_ctrl) <- colnames(ctrl_mean_filtert[1:25])
colnames(df_numeric_rnase) <- colnames(rnase_mean_filtert[1:25])

rownames(df_numeric_ctrl) <- rownames(ctrl_mean_filtert[1:25])
rownames(df_numeric_rnase) <- rownames(rnase_mean_filtert[1:25])
...
```{r}
# T-Test pro Zeilenpaar
t_test_ergebnisse1 <- lapply(seq_len(nrow(df_numeric_ctrl)), function(i) {
  ...
  # t-Test
    ...
    data.frame(
      ...
      p_wert = t_res$p.value,
      mittelwert_df_ctrl = mean(x, na.rm = TRUE),
      mittelwert_df_rnase = mean(y, na.rm = TRUE),
      t_wert = t_res$statistic,
      df = t_res$parameter
    )
  })
...
# Ergebnisse zusammenführen
t_test_ergebnisse_df1 <- do.call(rbind, t_test_ergebnisse1)
