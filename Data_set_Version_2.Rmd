...
# Proteine mit Abweichung ≥ 3 Fraktionen 
...
```{r}
# Erstelle den komplementären Data.Framem, der alle diejenigen Proteine beinhaltet, deren Hauptmaximum nicht um mindestens 2 Fraktionen shiftet, ctrl_main_shift "+" ctrl_no_main_shift = ctrl_norm_mean
...
correlations <- function(df1, df2, methode = "pearson") {
  ...
  for (i in 1:n) {
    correlations[i] <- cor(as.numeric(df1[i, 1:25]), as.numeric(df2[i, 1:25]), method = methode, use = "complete.obs")
  }

  abs_correlations = abs(correlations)
  
  mean_abs_correlation = mean(abs_correlations)
...
testing_correlations$mean_abs_correlaion
...
```{r}
ctrl_main_shift = ctrl_norm_mean
rnase_main_shift = ctrl_norm_mean 

#ctrl_same_max_amount = ctrl_3
#rnase_3_same_maxima_amount = rnase_3

#diff_index = ctrl_main_shift$only_max != rnase_3_selection$only_max
