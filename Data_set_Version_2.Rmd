...
***********************************************************************************************************
1. Kriterium der RNA-Abhängigkeit:
...
```{r}
# Mittelwert der Korrelationswerte (ctrl vs. rnase) von denjenigen Proteinen, deren Hauptmaximum um mindéstens 2 Fraktionen verschoben ist
testing_cor_shifter = correlations(ctrl_main_shift, rnase_main_shift)
testing_correlations$mean_abs_correlation

# Mittelwert der Korrelationswerte (ctrl vs. rnase) von denjenigen Proteinen, deren Hauptmaximum nicht um mindéstens 2 Fraktionen verschoben ist
...
testing_correlations$mean_abs_correlation < testing_cor_no_shifter$mean_abs_correlation 
```
...
*******************************************************************************************************************************
1.1 Identifizierung von Proteinen mit nur einem Maximum und deren Hauptmaxima um 2 Fraktionen abweichen zwischen CTRL und RNASE
*******************************************************************************************************************************

```{r}
# Hole die Protein- bzw. Zeilennamen aus ctrl_main_shift (enthält alle Proteine, deren Hauptmaxima um 2 Fraktionen abweicht)
IDs_main_shift <- rownames(ctrl_main_shift)

# Wähle nur die Protein IDs mit genau einem lokalen Maximum, das dem globalen Maximum entspricht
# id = jedes Protein
IDs_only1_maximum_shift <- IDs_main_shift[
  sapply(IDs_main_shift, function(id) {
    #Fraktion des lokalen Maxima des aktuellen Proteins wird in local_maxima gespeichert.
    local_maxima <- ctrl_norm_mean$local_maxima[[id]]
    #Fraktion des Hauotmaximas wird gespeichert.
    global_maximum <- ctrl_norm_mean$global_maximum[id]
    #Prüft: Ist nur eine Fraktionsnummer für lokale Maxima vorhanden und enstpricht diese der Fraktion des Hauptmaximums?
    #Aus ID_main_shift werden nur die IDs entnommen, bei denen hier TRUE ausgegeben wird. 
    length(local_maxima) == 1 && local_maxima == global_maximum
  })
]

# Filtere den DataFrame ctrl_main_shift auf diese Proteine
ctrl_only1_maximum_shift <- ctrl_main_shift[IDs_only1_maximum_shift, ]
```

```{r}
#Test, damit ich nicht versehentlich ein Protein verloren habe. Dieser Chunk sollte vor Abgabe gelöscht werden.
dim(ctrl_only1_maximum_shift)

IDs_main_shift <- rownames(ctrl_main_shift)

num_single_local_maxima <- sum(
  sapply(IDs_main_shift, function(id) {
    length(ctrl_norm_mean$local_maxima[[id]]) == 1
  })
)
num_single_local_maxima
```

*******************************************************************************************************************************
Identifizierung von Proteinen mit ungleicher Anzahl an Maxima und deren Hauptmaxima weniger als 2 Fraktionen abweichen zwischen CTRL und RNASE
*******************************************************************************************************************************

```{r}
#Hier liegt mein Handtuch!
  #Mein Platz!
  #D:
```
