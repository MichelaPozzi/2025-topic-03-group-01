...
**************************************************************************************************************
4. Find local maxima greater than 2 as fit parameters from average curves
**************************************************************************************************************
...
****************************************************************************************************************
5. Identify shoulders, regions that are not recognized as peaks, even though they have a high signal intensity.
****************************************************************************************************************
...
Initial pre-selection of proteins: A different number of peaks is a first indication of possible RNA-dependent proteins.
...
***********************************************************************************************************
Add COM values to ctrl_norm_mean

```{r}
#Selects the first protein intensities in 25 fractions.
#Goes through each row of ctrl_norm_mean.
#x = vector with the intensities of a protein across all 25 fractions for which the COM is currently being calculated
...
```{r}
# Create a new Data.Frame in which the main peak of the proteins is shifted, name: ctrl_main_shift, rnase_main_shift

# Calculate the difference (vector with absolute deviations)
diff_main_maxima <- abs(ctrl_norm_mean$global_maximum - rnase_norm_mean$global_maximum)

# Positions of proteins with deviation ≥ 3 fractions
deviating_main_maxima <- which(diff_main_maxima >= 2)

# length(deviating_main_maxima)
# 1351 proteins show a shift of the main maximum ≥ 2 fractions
...
```{r}
# Create the complementary Data.Frame that contains all proteins whose main maximum does not shift by at least 2 fractions, ctrl_main_shift "+" ctrl_no_main_shift = ctrl_norm_mean
...
```{r}
# Mean of the correlation values (ctrl vs. rnase) of those proteins whose main maximum is shifted by at least 2 fractions
testing_cor_shifter = correlations(ctrl_main_shift, rnase_main_shift)
testing_cor_shifter$mean_abs_correlation

# Mean of the correlation values (ctrl vs. rnase) of those proteins whose main maximum is not shifted by at least 2 fractions
...
Our hypothesis was confirmed. The mean correlation between ctrl and rnase is smaller for proteins whose main peak was shifted by at least two fractions. Here, it is 0.42. For proteins without a large shift of the main peak, it is 0.94.
...
=
The mean correlation for all proteins is 0.79. This is less than 0.94 and greater than 0.42.
...
*******************************************************************************************************************************
1. Criterion for data set purification
Identification of proteins from ctrl/rnase_no_main_shift (main peaks not significantly shifted) that exhibit a different number of total peaks (n_max_comb)


```{r}
# Extract vectors of the maximum number
...
# Create new DataFrame ctrl_equal_max with these proteins
ctrl_unequal_max <- ctrl_no_main_shift[unequal_max_proteins, ]
rnase_unequal_max <- rnase_no_main_shift[unequal_max_proteins, ]
```

*******************************************************************************************************************************
Proteins with the same number of total maxima (n_max_comb) from ctrl/rnase_no_main_shift
...
[Pefet fit - perfect multicollinearity with same variable used as predictor and target]
...
#Per PCA component a target variable is defined, so what should be predicted by the regression analysis
target_selected_pc1_1 <- pca_ctrl_pc1_pc2$PC1
