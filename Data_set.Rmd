...
By looking at these curves, it becomes obvious, that these proteins do not seem RNA-dependent. Despite their on-paper shift of the global maximum, their appearance is still very similar.  


We need an additional purification criteria for those proteins. Here, we use the center of mass. Even when there is a shift in the global maximum, it might be a microscopic shift regarding the actual intensity at the given fraction. Furthermore, the area around the shifted global maximum can still look very similar. Using the center of mass is suitable to identify these proteins, because those with similar areas around the peak, won´t show a great difference in their center of mass. 




Create new data.frames to select those proteins, that exhibit a shift in the global maximum AND a shift regarding their overall center of mass
```{r}
...
Visualization of the distribution of some proteins from ctrl/rnase_COM_shift
```{r}
# Vector with indices of proteins, that show a near zero correlation between ctrl and rnase
...
It is interesting, that there seems to be a few proteins, that exhibited a global maximum shift, but no COM shift, and still got a corrleation value aroung 0. How do they look?
```{r}
# Vector with indices of proteins, that show a near one correlation between ctrl and rnase
...
Interesting trend: In order to show global maximum shift, but no center of mass shift, the overall distribution needs to balance out. However, if there is a shift of the global maximum to the one side, and most of the intensity is located there, there needs to be another shift of the remaining intensity to the other side to balance out this effect. This seems to often occur for partially precipitated proteins, that show a left shift due to RNA-degradation and losing its natural cellular complex. On the other hand they precipitate through new interactions, leading to the overall correlation value of 0. These proteins are still considered RNA-dependent. 
...
In order to isolate them, we look at ctrl_no_COM_shift and rnase_no_COM_shift and seperate thoe proteins, that show a different amount of total maxima (n_max_comb) between ctrl and rnase 

```{r}
#ctrl_no_COM_shift
#rnase_no_COM_shift

# Extract vectors with the maximum numbers
n_max_ctrl = ctrl_no_COM_shift$n_max_comb
n_max_rnase = rnase_no_COM_shift$n_max_comb

# Find proteins (row names) with the same number of maxima 
equal_max_proteins = rownames(ctrl_no_COM_shift[n_max_ctrl == n_max_rnase])

# Find proteins (row names) with a different number of maxima
unequal_max_proteins = rownames(ctrl_no_COM_shift[n_max_ctrl != n_max_rnase])

# New data.frames for the proteins with same numer of maxima 
ctrl_no_diff_max <- ctrl_no_COM_shift[equal_max_proteins, ]
rnase_no_diff_max <- rnase_no_COM_shift[equal_max_proteins, ]

# New data.frames for the proteins with different number of maxima 
ctrl_diff_max <- ctrl_no_COM_shift[unequal_max_proteins, ]
rnase_diff_max <- rnase_no_COM_shift[unequal_max_proteins, ]
```
