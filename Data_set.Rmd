...
Part 2
9. Criteria for selecting RNA-dependent proteins
...
###########################################################################
Teil von Julian Baureis (Zeile 701 - 1290), Codewriting assisted by ChatGPT 
###########################################################################

***********************************************************************************************************
9. Useful functions, that will be used in this part 
***********************************************************************************************************
...
```
The proteins did not show a COM shift. However, their overall distribution looks different. Often, one curve shifts a bit to both sides at the same time, resulting in this effect. 
...
Question: Are the isoelectric points of the selected proteins greater than 7?
...
par(mfrow = c(1, 1))
```
The selected proteins (left histogram) show a tendency to higher values for the isoelectric point. The mean value (green line) of the isoelectric points is higher in this histogram than in the other histogram, and differs more from the neutral value (yellow line). There are in general more values for higher isoelectric points among the selected proteins. Based on our intensity distribution criteria we managed to separate proteins we assumed are RNA-dependent. This assumption seems to be in accord with the observed behaviour of the histogramms, in which our likely RNA-dependent proteins tend to show a higher isoelectric point.
...
abline(a = 0, b = 1, col = "firebrick", lwd = 3, lty = 3)
...
par(mfrow = c(1, 1)) 
```
Interpretation: For the not selected proteins, there is mostly no or only a small deviation between the COM values when comparing ctrl and rnase intensities.  
For the selected proteins, there is a clear deviation between the COM values when comparing ctrl and rnase. There is a trend observable. Most of the data points show a high COM value in the ctrl group, and at the same time, a lower COM value for the rnase group. This indicates, that during the RNse digestion in the experiment, those proteins previously bound to RNA were freed and isolated from their RNP-complexes and migrated into less dense fractions. A big part of their intensity "potential" and hence their center of mass consequently appears in smaller fractions, leading to the observed lower COM values for the rnase group.



#####################################
Julian Baureis Teil des Codes zu Ende 
#####################################

```{r}
# Calculate the absolute difference of the positions of the COM values between ctrl and rnase 
diff_test <- (ctrl_selected$COM - rnase_selected$COM)

# Indices of proteins with deviation ≥ 3 fractions
deviating_test <- which(diff_test >= 3)

# Apply on the data.frames
ctrl_test_shift = ctrl_selected[deviating_test, ]
rnase_test_shift = rnase_selected[deviating_test, ]

# Create the complementary data.frame that contains all proteins whose COM do not shift by at least 2 fractions
ctrl_no_test_shift = ctrl_selected[-deviating_test, ]
rnase_no_test_shift = rnase_selected[-deviating_test, ]
```
...
# Falls nötig: nur die ersten 25 Spalten auswählen
rnase_test_shift_25 <- rnase_test_shift[, 1:25]
rnase_no_test_shift_25 <- rnase_no_test_shift[, 1:25]

# Gemeinsame Proteine sicherstellen (optional, falls Spalten unterschiedlich)
common_proteins <- intersect(colnames(rnase_test_shift_25), colnames(rnase_no_test_shift_25))
rnase_test_shift_25 <- rnase_test_shift_25[, common_proteins]
rnase_no_test_shift_25 <- rnase_no_test_shift_25[, common_proteins]

# Daten kombinieren
combined <- rbind(rnase_test_shift_25, rnase_no_test_shift_25)

# Labels erstellen
labels <- c(rep("rnase_test_shift", nrow(rnase_test_shift_25)), rep("rnase_no_test_shift", nrow(rnase_no_test_shift_25)))

# Daten standardisieren
combined_scaled <- scale(combined)

# PCA durchführen
pca_res <- prcomp(combined_scaled, center = FALSE, scale. = FALSE)

# PCA-Ergebnisse als Dataframe für Plot
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = Group)) +
  ...
  labs(title = "PCA: rnase_test_shift vs rnase_no_test_shift",
       x = paste0("PC1 (", round(summary(pca_res)$importance[2,1]*100,1), "% Varianz)"),
       y = paste0("PC2 (", round(summary(pca_res)$importance[2,2]*100,1), "% Varianz)")) +
  theme_minimal() +
...
# Nur die ersten 25 Spalten auswählen
ctrl_test_shift_25 <- ctrl_test_shift[, 1:25]
rnase_test_shift_25 <- rnase_test_shift[, 1:25]

# Gemeinsame Proteine (Spalten) sicherstellen
common_proteins <- intersect(colnames(ctrl_test_shift_25), colnames(rnase_test_shift_25))
ctrl_test_shift_25 <- ctrl_test_shift_25[, common_proteins]
rnase_test_shift_25 <- rnase_test_shift_25[, common_proteins]

# Daten kombinieren
combined <- rbind(ctrl_test_shift_25, rnase_test_shift_25)

# Labels erstellen
labels <- c(rep("ctrl_test_shift", nrow(ctrl_test_shift_25)),
            rep("rnase_test_shift", nrow(rnase_test_shift_25)))
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = Group)) +
  geom_point(alpha = 0.8, size = 3) +
  labs(title = "PCA: ctrl_test_shift vs rnase_test_shift",
       x = paste0("PC1 (", round(summary(pca_res)$importance[2,1]*100,1), "% Varianz)"),
       y = paste0("PC2 (", round(summary(pca_res)$importance[2,2]*100,1), "% Varianz)")) +
  theme_minimal() +
...
Evaluation of the Selection Criteria
  by Luisa Wintel (and Julian) influenced by ChatGPT
...
```{r}
# Julians custom plotting function 'header_plotting'.
# It likely highlights the top 100 proteins with high correlation (< 0.99) between two datasets.
header_plotting(cor_all_proteins$correlations, # correlation values
                ctrl_norm_mean, # condition 1
                rnase_norm_mean, # condition 2
                n =100, # how many top results to show
                cor_threshold = 0.99, # only correlations less than 0.99
                direction = "lt") # less than
```
