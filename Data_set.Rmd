...
# Compares each value in columns 1 to 25 (the fractions) of the rnase_norm_mean dataframe with the threshold of 2%.
...
```{r}
# Removes and excludes 3 fractions left and right of each peak (7 in total) from the shoulder search.
...
th_max_reg_simple <- function(x) {
  peaks <- as.numeric(unlist(x$maxima)) # Converts the maxima column from *_norm_mean to a numeric vector.
  mask <- rep(1, 25)
...
# Applies the function row by row to the entire rnase_3 or ctrl_3 DataFrame.
...
peaks_regions = function(x) {
  rle_result = rle(as.numeric(x[1:25])) # rle() finds all contiguous equivalent sequences
...
***********************************************************************************************************
```{r}
# Get the total number of maxima for RNASE and CTRL
