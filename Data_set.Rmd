...
rnase_3$fit_c <- apply(vect, 2, function(t) {
                          ...
                          y <- c(1:25)
                          data <- data.frame(y = y, df.y = df.y)#individual dataframe created for every protein 
                          Mean_rnase_list <- as.numeric(unlist(rnase_3[t,"rnase_max"]))#vector with all peak positions
                          ...
                          # Before optimization of the parameters, check the number of maxima for the profile of the protein
												  if (n==0) { gauss <- 0 } else {
												  				  								if (n==1) {
												  		  	         									  ...
												  		  	         									  #Vektor = (Max. Intensitätswert, Position dieses Wertes, 1)
