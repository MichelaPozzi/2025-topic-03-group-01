...
```{r}
# Goal: Functions to optimize for Gaussian fitting - Gaussians with 1 to 6 peaks
...
f1 <- function(data, q) {
					...
					sigma1 <- q[3]  #Standard deviation for the gauss function, that will be created 
					
					#Difference between values for theoretical gauss function and the measured values stored in data$df.y for the 25 fractions = Residuals. Gauss function is used for discrete values at these positions 
					res <- (C1 * exp(-(data$y-mean1)**2/(2 * sigma1**2))) - data$df.y 
					sum(res * res)  #Residuals squared
          }

#Add the two created gauss functions to gain a joined function, that describes both peaks. Values of the joined function are defined by the sum of the two values originating from the two gauss functions.
f2 <- function(data, q) {
					C1 <- q[1]
					mean1 <- q[2]
					sigma1 <- q[3]
...
# Goal: Get the amplitude of the fit at each peak - new column (31): fit_c
# $par[1]: Identifies amplitude of peak

#******** Notes ********												  		  	         									  
# optim(values fo optimization, function to be minimized, data, method: Broyden–Fletcher–Goldfarb–Shanno algorithm, control of tolerance and accuracy) changes the parameters (of "vector"!) systematically to minimize the error.
# optim returns a result object with, among other things, the best parameter set (fit$par) and the minimum value of the function.
...
rnase_3$fit_c <- apply(vect, 2, function(t) {
                          ...
                          y <- c(1:25) # y = fraction (x-axis)
                          data <- data.frame(y = y, df.y = df.y) # data describes the signal distribution of a protein across the fractions.
                          Mean_rnase_list <- as.numeric(unlist(rnase_3[t,"rnase_max"])) # mean_rnase_list contains peak positions (vektor).
                          n <- length(Mean_rnase_list) # Determine how many peaks/shoulders were found.
...
```{r}
# Fit the RNASE mean curves 
# Goal: Get the position of the peak(s) - new column (32): fit_mean
...
```{r}
# Fit the RNASE mean curves 
# Goal: Get the covariance (sigma) of the peak(s) - new column (33): fit_sigma
...
```{r}
# Fit the RNASE mean curves 
# Goal: Get the residual sum of the squares - new column (34): fit_res
# $value: Identifies the minimal error found by optim with the altered (optimized) parameters
# --> Allows statements to be made about the qiality of the fitting
