...
Corrleation (method pearson) is used to identify how similar or different the intensity distribution curves for a protein between ctrl and rnase is. Low correlation means less similar curves, high correlation means more similar curves.

```{r}
# Function to calculate the row-wise correlation
...
Get the variables for the correlation values 
```{r}
...
The mean correlation for all proteins is 0.79. This is less than 0.92 and greater than 0.36.
This initially suggests that purification of RNA-dependent proteins based on the criterion of a significant shift in the center of mass appears to be reasonable. Our goal was to select less similar curves and separate them from similar ones.





Plotting the correlation values as a histogram
```{r}
par(mfrow = c(1, 2)) 
...
par(mfrow = c(1, 1))
```
Both histograms contain a part of the proteins, that exhibited a shift in their center of mass. The correlation values for proteins with a shifted COM are much more located around 0. The correlation values for proteins with no COM shift show a clear tendency to high correlation values, most of them for correlation values between 0.9 and 1. 



Look at those proteins, despite showing a shift in their center of mass, that still show a high correlation value
```{r}
# Vector with indices of proteins, that show a near one correlation between ctrl and rnase
near_one_cors = which(abs(COM_shifter$correlations) <= 0.95) 
...
```
Maybe a reason for wrong positives: The proteins that have a big maximum, which is slightly shifted. As a consequence, the COM will show a shift, even tho the general appareance of the curve remains similar. 
...
Look at the proteins with no big COM shift, that have a very high correlation value. 
```{r}
...
```
As expected, these proteins have a very similar distribution. They should not be considered RNA-dependent. But what about the proteins, that show no COM shift, but still a low correlation value. How do they look?



Look at the proteins with no big COM shift, that have a lower correlation value
```{r}
# Vector with indices of proteins, that show a near one correlation between ctrl and rnase
lower_cors = which(abs(COM_no_shifter$correlations) < 0.6) 

indices = lower_cors[1:5]
...
```
For these particular proteins there was a trend observable. In order to not show a center of mass shift, but still have a lower correlation value, the overall distribution needs to balance out. The rnase curve seems to drift to both sides of the ctrl curve. In many cases, a big part of the intensity of the rnase curve is found at the last fractions, indicating a precipitating behavior of these proteins in the rnase experiment. In order not to show a COm shift, the other part of the intensity shifts to lower fractions. These proteins should still be considered RNA-dependent. 




We use a shift in the global maximum as another criterion, to identify the "two-sided" shifting proteins. Despite them not changing their COM value, they should still show a shift, when only observing their greatest maximum. 
```{r}
...
```
The mean correlation value for the proteins with a COM shift AND an additional shift in their global maximum is much lower than the mean correlation value for those proteins with no COM shift and no global maximum shift. The correlation values are separately plotted in a histogram. 
...
Define an arbitrary threshold of 0.7 for the histogram, in which the proteins with no shift in their center of mass are shown, in order to separate those with likely similar appearing curves (cor > 0.7) from the rest
...
5: Check, if the combination of both data.frames would still consits of all the proteins from the start data.frames. If the result is TRUE, then everything worked. 

```{r}
dim(ctrl_not_selected)[1] + dim(ctrl_selected)[1] == dim(ctrl_norm_mean)[1]
```
(maybe Com differences, or even something completely different for the statistical test?!)
...
```
Interpretation: The density for the differences for selected is greater for positive values. Therefore, the following largely seems to hold: The mean values of the control maxima are larger than the mean values of the RNase maxima, which may indicate an increased left shift of the RNase fractions.
