...
###### Supervisors: Caudron-Herger, M., Pozzi, M. 
...
2. Data cleanup - Rearrange and reorder the columns to their treatment, replicate and fraction
  by Julia Ferdin influenced Ms_Statistical_Analysis (Maïwen Caudron-Herger)
...
**************************************************************************************************************
4. Find Local Maxima
  by Julian Baureis & Luisa Wintel influenced by ChatGPT & Ms_Statistical_Analysis (Maïwen Caudron-Herger)
...
(by Julian Baureis, assisted by ChatGPT)
...
Correlation may be seen as a measure for the similarity between the distribution of two curves. 
...
The curves seem to look very similar, which may explain the high correlation value. Some of the proteins seem to partially shift, if for example ctrl (blue) and rnase (red) have a joined big peak, and the blue (ctrl) curve has an additional small peak.
...
As expected, these proteins seem to have a very similar distribution. They should not be considered RNA-dependent. But what about the proteins, that show no big COM shift, but still a low correlation value. How do they look?
...
The proteins did not show a COM shift. However, their overall distribution seems to look different. Often, one curve shifts a bit to both sides at the same time, resulting in the effect of no COM shift.
...
hist(
  ...
  main = "Isoelectric points: selected proteins",
...
hist(
  ...
  main = "Isoelectric points: all proteins",
  xlab = "Isoelectric point",
  ylab = "Frequency",
  breaks = 30,
  col = "grey70",
...
Perform again for the not selected proteins
```{r}
# Create a new dataframe, that only contains those proteins from our not selected group, that were also listed in the external data rbp_pos & rbp_neg, as well as the respective isoelectric points for these proteins.

# Protein names of all selected proteins 
protein_names = rownames(ctrl_not_selected)
...
# Get the subsets for the included proteins
df_pos = ctrl_not_selected[indices_pos, ]
df_neg = ctrl_not_selected[indices_neg, ]
...
# Combine to new dataframes, with Protein Names and their isoelectric point 
df_pos_combined_not_selected = data.frame(protein = matched_pos_names, isoelectric_point = df_pos_pI)
df_neg_combined_not_selected = data.frame(protein = matched_neg_names, isoelectric_point = df_neg_pI)

# Merge those two dataframes together 
final_df_combined_not_selected = rbind(df_pos_combined_not_selected, df_neg_combined_not_selected)
```


Third t-test is performed (one-sided, right-tailed)
Question: Are the isoelectric points of the selected proteins significantly higher than those of the not selected proteins?

--> H0: The mean isoelectric point for the selected proteins is less than or equal to the mean isoelectric point for the not selected proteins 

--> H1: The mean isoelectric point for the selected proteins is greater than the mean isoelectric point for the not selected proteins     

```{r}
# Perform the third t test
t.test(final_df_combined_selected$isoelectric_point, final_df_combined_not_selected$isoelectric_point, alternative = "greater")
```
The p-value is << 0.05, and the H1 hypothesis is accepted, H0 is rejected. The isoelectric points for the selected proteins are significantly greater than the isoelectric points for the not selected proteins.
...
hist(
  final_df_combined_not_selected$isoelectric_point,
  main = "Isoelectric points: not selected proteins",
  xlab = "Isoelectric point",
  ylab = "Frequency",
  breaks = 30,
  col = "mediumpurple1",
...
abline(v = mean(final_df_combined_not_selected$isoelectric_point, na.rm = TRUE),
       col = "black", lwd = 4)

par(mfrow = c(1, 1))
```
The different distributions of the isoelectric points between the selected and not selected proteins are clearly visible in this histogram.
...
It should be further noted, that among our selected proteins, there are also a few with no big COM shift. They are marked in red. They mostly show their COM value for ctrl and rnase in higher fractions. Per definition, these proteins have a shift in their main maximum between ctrl and rnase, and also have a correlation value < 0.7 between ctrl and rnase. One possible explanation for the combination of these properties (main maximum shift + lower correlation value + no big COM shift) may be, that the proteins are partially shifting left and right in the rnase group compared to the ctrl group. If one part of the protein distribution moves in lower fractions and the other part in higher fractions, the overall COM value is not changed that much, but there may be a shift in the main maximum and a reduced correlation value.
...
15. Evaluation of the Selection Criteria
  by Luisa Wintel influenced by ChatGPT
...
```{r}
#kmeans is executed with 3 centers
...
The regression analyses for the not-selected proteins describe its own target variable (ctrl_not_selected$global_maximum) also very well. 

Sometimes the not-selected proteins describe their own target variable even better than the selected proteins describe theirs, as there are more proteins used in the model of the not-selected proteins. However, the analysis for the not-selected proteins shows a bad fit for the selected target variable (ctrl_selected$global_maximum). 

This proves, that there is a difference between the selected and not-selected proteins, and therefore, that the selection criteria worked.
