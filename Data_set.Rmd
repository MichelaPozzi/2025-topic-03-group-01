...
###########################################################################
Teil von Julian Baureis (Zeile 701 - 1218), Codewriting assisted by ChatGPT 
###########################################################################
...
Visualization of the proteins from ctrl_no_CoM_shift and rnase_no_COM_shift (shift in global maximum, no shift in the COM), that show a low correlation value
```{r}
# Vector with indices of proteins, that show a near zero correlation between ctrl and rnase
...
Visualization of the proteins from ctrl_no_CoM_shift and rnase_no_COM_shift (shift in global maximum, no shift in the COM), that show a high correlation value
```{r}
# Vector with indices of proteins, that show a near one correlation between ctrl and rnase
...
The plotted protein curves show the same problem as discussed earlier. On paper, their global maximum shifted, but their center of mass didnt. In comparison to those proteins plotted before based on the same histogram, that had low correlation values and did not look similar, these protein curve with high correlation values look very similar. Their peak regions look very similar over all. They are not classified as RNA-dependent in this analysis.
...
```
Interpretation: The widest point of the differences of "selected" is at positive values. Therefore, the following largely seems to hold: The mean values of the control maxima are larger than the mean values of the RNase maxima, which may indicate an increased left shift of the RNase fractions.
