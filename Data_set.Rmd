...
#rnase_clean <- as.data.frame(lapply(rnase_mean_filtert_final, function(col) unlist(col)))
```
...
# Ergebnisliste
prop_tests <- lapply(seq_len(nrow(df1)), function(i) {
  x <- c(df1$erfolge[i], df2$erfolge[i])     # Erfolge aus beiden Gruppen
  n <- c(df1$gesamt[i], df2$gesamt[i])       # Versuchszahlen aus beiden Gruppen
...
prop_test_df <- do.call(rbind, prop_tests)
...
# Oder mit ggplot2
library(ggplot2)
pca_df <- as.data.frame(pca_result_ctrl$x)
