...
f1 <- function(data, q) {
					C1 <- q[1]        # amplitude
					mean1 <- q[2]     # position of peak
					sigma1 <- q[3]    # standard deviation
					# Calculate the difference (residuals) between the theoretical Gaussian curve and the measured values data$df.y.
					res <- (C1 * exp(- (data$y-mean1)**2/(2 * sigma1**2))) - data$df.y # data$df.y = measured intensities in each fraction for the corresponding protein.
					sum(res * res)    # Measure of how well the theoretical peak fits the data.
				 }
