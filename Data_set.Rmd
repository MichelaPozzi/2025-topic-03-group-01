...
***********************************************************************************************************
3. Combining the purified data sets back together for further analysis
***********************************************************************************************************
...
t.test(final_df_combined$isoelectric_point, mu = 7.0, alternative = "greater")
```

We get a very high value for the test statistic (19,4), and a clearly significant p-value (<< 0.05). The H0-hypothesis is rejected, and the H1-hypothesis is statistically proven. The mean isoelectric point of the selected proteins is significantly greater than the neutral value of 7.




```{r}
hist(final_df_combined_selected$isoelectric_point, breaks = 50)
abline(v = mean(final_df_combined_selected$isoelectric_point), col = "red")
```
...
# How many proteins were not listed in either rbp_pos or rbp_neg, but still used in our data set - 235
dim(ctrl_norm_mean)[1] - dim(final_df_combined_not_selected)[1]
```

```{r}
# Perform the second t-test
t.test(final_df_combined_not_selected$isoelectric_point, mu = 7.0, alternative = "two.sided")
```

```{r}
hist(final_df_combined_not_selected$isoelectric_point, breaks = 50)
abline(v = mean(final_df_combined_selected$isoelectric_point), col = "red") 
```
...
***********************************************************************************************************
5. Further visualizations and interpretation of the purified data
***********************************************************************************************************
...
par(mfrow = c(1, 1)) 
```
Interpretation: For the not selected proteins, there is mostly no or only a small deviation for the COM values when comparing ctrl and rnase intensities.  
For the selected proteins, there is a clear deviation for the COM values when comparing ctrl and rnase. There is a clear trend observable. Most of the data points show a high COM value in the ctrl group, and at the same time, a lower COM value for the rnase group. This indicates, that during the RNse digestion in the experiment, those proteins previously bound to RNA were freed and isolated from their RNP-complexes and migrated into less dense fractions. They then appear more in the smaller fractions, which leads to the observed lower COM values for the rnase group.
...
# Calculate mean values of the combined maxima (local maxima + shoulders) per row and form the difference
...
The density for the differences for "selected" is more even distributed, but reaches far greater values, which is expected, because, under our criteria, these proteins should be not similar between their ctrl and rnase curves. 
